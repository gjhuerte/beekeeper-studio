...
export async function selectTopStream(conn, db, table, orderBy, filters, chunkSize) {
  ...
  return {
    totalRows: rowCount,
    columns,
    cursor: new MongodbCursor(conn, query, params, chunkSize)
  }
}
    ...
    execute() {
      return runWithConnection(conn, async (connection) => {
        ...
        try {
          ...
          if (canceling && err.code === mongodbErrors.CONNECTION_LOST) {
            canceling = false;
...
async function alterTableSql(conn, change) {
  ...
  const builder = new MongodbChangeBuilder(change.table, columns)
...
export function alterIndexSql(payload) {
  ...
  const changeBuilder = new MongodbChangeBuilder(table, schema, [])
...
export function alterRelationSql(payload) {
  ...
  const builder = new MongodbChangeBuilder(table, schema, [])
...
function driverExecuteQuery(conn, queryArgs) {
  ...
  const runQuery = (connection) => new Promise((resolve, reject) => {
    connection.query({ sql: queryArgs.query, values: queryArgs.params, rowsAsArray: queryArgs.rowsAsArray }, (err, data, fields) => {
      if (err && err.code === mongodbErrors.EMPTY_QUERY) return resolve({});
